...
class CalibrationCam(object):
    ...
    def cali_cam(self, cal_path):
        ...
        print("cv2.calibrateCamera LEFT start")
        rt_left, self.M1, self.d1, self.r1, self.t1 = cv2.calibrateCamera(
            self.objpoints, self.imgpoints_l, img_shape, None, None)
        self.rt_left = rt_left

        print("cv2.calibrateCamera RIGHT start")
        rt_right, self.M2, self.d2, self.r2, self.t2 = cv2.calibrateCamera(
            self.objpoints, self.imgpoints_r, img_shape, None, None)

        self.rt_right = rt_right
        ...
        with open(self.cal_path + "/calibCam" + '_' + self.num[:7] + ".json", "w", encoding='utf-8') as f:
...
class StereoCalibration(object):
    ...
    def read_calibMat(self, cal_path):
        # 读取json文件
        with open(cal_path + '/calibCam' + '_' + self.num[:7] + '.json', 'r') as f:
    ...
    def stereo_cal(self, cal_path):
        ...
        with open(self.cal_path + "/calibMatTest" + '_' + self.num[:7] + ".json", "w", encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_num', default='05', help='base_num as 00 01 02 …… 99')

    # parser.add_argument('--filepath', default=r'./calibrate/' + base_num, help='String Filepath')
    # parser.add_argument('--filepath', default=r'./calibrate/test/', help='String Filepath')
    args = parser.parse_args()
    base_num = 'base_' + args.base_num + r'/'
    cal_data = CalibrationCam(base_num, args.filepath, args.gridDot)
    cal_data2 = StereoCalibration(base_num, args.filepath, args.gridDot)
    os.system("pause")
    os.system("pause")
